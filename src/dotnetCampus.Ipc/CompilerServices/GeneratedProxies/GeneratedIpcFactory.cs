...
public static class GeneratedIpcFactory
{
    ...
    public static void RegisterIpcPublic<TContract>(Func<object>? proxyFactory, Func<object>? jointFactory)
        where TContract : class
    {
        IpcFactories[typeof(TContract)] = (proxyFactory, jointFactory);
    }
    ...
    public static TContract CreateIpcProxy<TContract>(this IIpcProvider ipcProvider, IPeerProxy peer, string? ipcObjectId = null)
        where TContract : class
    {
        if (IpcFactories[typeof(TContract)].ProxyFactory is not { } proxyFactory)
        {
            throw new ArgumentException(
                $"接口 {typeof(TContract).Name} 上没有找到 {nameof(IpcPublicAttribute)} 特性，因此不知道如何创建 {typeof(TContract).Name} 的 IPC 代理。",
                nameof(TContract));
        }

        var proxy = (GeneratedIpcProxy<TContract>)proxyFactory();
    ...
    /// <param name="ipcObjectId">如果要调用的远端对象有多个实例，请设置此 Id 值以找到期望的实例。</param>
    /// <returns>契约类型。</returns>
    public static TContract CreateIpcProxy<TContract>(this IIpcProvider ipcProvider, IPeerProxy peer, IpcProxyConfigs ipcProxyConfigs,
        ...
        where TContract : class
    {
        ...
        {
            throw new ArgumentException(
                $"类型 {typeof(TShape).Name} 上没有找到 {nameof(IpcShapeAttribute)} 特性，因此不知道如何创建 {typeof(TContract).Name} 的 IPC 代理。",
                nameof(TShape));
        }
        ...
        where TContract : class
    {
        ...
        if (IpcFactories[typeof(TContract)].JointFactory is not { } jointFactory)
        {
            throw new ArgumentException(
                $"类型 {realType.Name} 上没有找到 {nameof(IpcPublicAttribute)} 特性，因此不知道如何创建 {typeof(TContract).Name} 的 IPC 对接。",
                nameof(realInstance));
        }

        var context = ipcProvider.GetGeneratedContext();
        var joint = (GeneratedIpcJoint<TContract>)jointFactory();
